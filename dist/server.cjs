...
var SUPABASE_URL = (process.env.SUPABASE_URL || "").replace(/^["']|["']$/g, "").trim();
var SUPABASE_KEY = (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || "").replace(/^["']|["']$/g, "").trim();
var GOOGLE_CLIENT_ID = (process.env.GOOGLE_CLIENT_ID || process.env.VITE_GOOGLE_CLIENT_ID || "").replace(/^["']|["']$/g, "").trim();
var SESSION_SECRET = (process.env.SESSION_SECRET || SUPABASE_KEY || GOOGLE_CLIENT_ID).replace(/^["']|["']$/g, "").trim();
var STORAGE_BUCKET = (process.env.SUPABASE_STORAGE_BUCKET || "pin-images").replace(/^["']|["']$/g, "").trim();
var isSupabaseConfigured = SUPABASE_URL.length > 0 && SUPABASE_KEY.length > 0;
var isGoogleAuthConfigured = GOOGLE_CLIENT_ID.length > 0;
var isSessionConfigured = SESSION_SECRET.length >= 32;
var supabase = isSupabaseConfigured ? (0, import_supabase_js.createClient)(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
...
var publicApiRateLimit = rateLimit(6e4, 180);
var writeApiRateLimit = rateLimit(6e4, 60);
var authApiRateLimit = rateLimit(6e4, 20);
var uploadApiRateLimit = rateLimit(60 * 6e4, 40);
...
function signValue(value) {
  return base64UrlEncode((0, import_crypto.createHmac)("sha256", SESSION_SECRET).update(value).digest());
}
...
function verifySessionToken(token) {
  ...
  if (actualBuffer.length !== expectedBuffer.length || !(0, import_crypto.timingSafeEqual)(actualBuffer, expectedBuffer)) {
    return null;
  }
  try {
...
function stringField(value, field, max, required = true) {
...
function sanitizeTags(value) {
  ...
  return value.map((tag) => typeof tag === "string" ? tag.trim().toLowerCase().replace(/[^a-z0-9_-]/g, "") : "").filter(Boolean).slice(0, 10).map((tag) => tag.slice(0, 32));
}
...
function mapPin(p, savedMap = {}) {
  return {
    ...
    aspectRatioValue: Number(p.aspect_ratio_value) || 1,
...
function validatePinInput(value, userId) {
  if (!isRecord(value)) throw new Error("Pin payload is invalid.");
  return {
    id: idField(value.id || `pin_${Date.now()}_${(0, import_crypto.randomBytes)(4).toString("hex")}`, "pin.id"),
    ...
    createdAt: stringField(value.createdAt || (/* @__PURE__ */ new Date()).toISOString(), "Pin creation date", 60, false)
  };
}
function validateBoardInput(value) {
  if (!isRecord(value)) throw new Error("Board payload is invalid.");
  const pinIds = Array.isArray(value.pinIds) ? value.pinIds.map((pinId) => idField(pinId, "board.pinIds", false)).filter(Boolean).slice(0, 300) : [];
  return {
    id: idField(value.id || `board_${Date.now()}_${(0, import_crypto.randomBytes)(4).toString("hex")}`, "board.id"),
    name: stringField(value.name, "Board name", 80),
    pinIds,
    createdAt: stringField(value.createdAt || (/* @__PURE__ */ new Date()).toISOString(), "Board creation date", 60, false)
  };
}
function validateCreatorInput(value, userId) {
  ...
  return {
    ...
    avatar: httpUrlField(value.avatar, "Profile avatar", 3e3),
    ...
    followersCount: numberField(value.followersCount, 0, 0, 1e6),
    followingCount: numberField(value.followingCount, 0, 0, 1e6),
    savesCount: numberField(value.savesCount, 0, 0, 1e6)
  };
}
async function upsertCreatorProfile(creator) {
  if (!supabase) return;
...
async function getSavedMap(pinIds) {
  if (!supabase) return {};
...
async function ensureBoardOwnership(boardId, userId) {
  if (!supabase) return false;
  const { data, error } = await supabase.from("boards").select("creator_id").eq("id", boardId).maybeSingle();
  if (error) throw error;
  return !data || data.creator_id === userId;
}
async function ensurePinOwnership(pinId, userId) {
  if (!supabase) return false;
  const { data, error } = await supabase.from("pins").select("creator_id").eq("id", pinId).maybeSingle();
  if (error) throw error;
  return !data || data.creator_id === userId;
}
...
app.post("/api/auth/google", authApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    const username = `${usernameBase}_${String(tokenInfo.sub).slice(-6)}`.toLowerCase().replace(/[^a-z0-9_]/g, "_").replace(/_+/g, "_").replace(/^_|_$/g, "");
    const creator = {
      id: `google_${tokenInfo.sub}`,
      name: tokenInfo.name || usernameBase,
      username,
      avatar: tokenInfo.picture || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=150&h=150",
...
app.get("/api/creators", async (_req, res) => {
  ...
  try {
    const { data, error } = await supabase.from("profiles").select("*").order("followers_count", { ascending: false });
...
app.get("/api/pins", async (req, res) => {
  ...
  try {
    ...
    let query = supabase.from("pins").select("*").order("created_at", { ascending: false }).limit(limit + 1);
    if (cursor) {
    ...
    const savedMap = await getSavedMap(pageRows.map((p) => p.id));
    const pins = pageRows.map((p) => mapPin(p, savedMap));
...
app.post("/api/pins", writeApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    const createdAt = pin.createdAt || (/* @__PURE__ */ new Date()).toISOString();
    ...
    const { data, error } = await supabase.from("pins").upsert({
      id: pin.id,
      title: pin.title,
    ...
    if (error) throw error;
    await supabase.from("saved_pins").upsert({ user_id: claims.userId, pin_id: pin.id }, { onConflict: "user_id,pin_id" });
...
app.post("/api/uploads/image", uploadApiRateLimit, async (req, res) => {
  ...
  if (!claims) return;
  try {
    const dataUrl = stringField(req.body.dataUrl, "Image", 12e6);
    ...
    const safeName = fileName.replace(/\.[^/.]+$/, "").toLowerCase().replace(/[^a-z0-9_-]/g, "-").replace(/-+/g, "-").slice(0, 60);
    const storagePath = `${claims.userId}/${Date.now()}-${(0, import_crypto.randomBytes)(6).toString("hex")}-${safeName || "image"}.${extension}`;
    ...
    res.status(400).json({
      error: err.message || `Image upload failed. Confirm the "${STORAGE_BUCKET}" Supabase Storage bucket exists and is public.`
    });
  }
});
...
app.post("/api/boards", writeApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from("boards").upsert({
      ...
      created_at: board.createdAt || (/* @__PURE__ */ new Date()).toISOString()
    }).select("*").single();
    if (error) throw error;
...
app.post("/api/save-relation", writeApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    if (isSaved) {
      await supabase.from("saved_pins").upsert({ user_id: claims.userId, pin_id: pinId }, { onConflict: "user_id,pin_id" });
    } else {
...
app.post("/api/follow-relation", writeApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    if (isFollowing) {
      await supabase.from("follows").upsert({ follower_id: claims.userId, following_id: followingId }, { onConflict: "follower_id,following_id" });
    } else {
...
app.get("/api/user-data/:userId", async (req, res) => {
  ...
  try {
    ...
    const { data: followsData, error: followsError } = await supabase.from("follows").select("following_id").eq("follower_id", userId);
    const { data: savedData, error: savedError } = await supabase.from("saved_pins").select("pin_id").eq("user_id", userId);
    if (followsError || savedError) {
      return res.json({ followedCreatorIds: [], savedPinIds: [] });
    }
    res.json({
      followedCreatorIds: followsData ? followsData.map((f) => f.following_id) : [],
      savedPinIds: savedData ? savedData.map((s) => s.pin_id) : []
    });
  } catch (err) {
...
app.get("/api/pins/:pinId/comments", async (req, res) => {
  ...
  try {
    ...
    const { data: commentsData, error } = await supabase.from("comments").select("*").eq("pin_id", pinId).order("created_at", { ascending: false }).limit(100);
    if (error || !commentsData) return res.json([]);
    const creatorIds = Array.from(new Set(commentsData.map((comment) => comment.creator_id)));
    const { data: profiles } = creatorIds.length ? await supabase.from("profiles").select("id, name, avatar").in("id", creatorIds) : { data: [] };
    ...
    res.json(
      commentsData.map((comment) => {
        ...
        return {
          ...
          creatorAvatar: profile?.avatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=150&h=150",
          text: comment.text,
...
app.post("/api/pins/:pinId/comments", writeApiRateLimit, async (req, res) => {
  ...
  try {
    ...
    const id = `comment_${Date.now()}_${(0, import_crypto.randomBytes)(4).toString("hex")}`;
    ...
    if (error) throw error;
    const { data: profile } = await supabase.from("profiles").select("name, avatar").eq("id", claims.userId).maybeSingle();
    res.json({
      id,
      pinId,
